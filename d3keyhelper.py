...
try:
    from .platform_compat import (  # noqa: F401
        read_process_commandline, looks_like_proton_diablo_window, format_window_debug,
        play_notification_sound, set_dpi_awareness,
        ActiveWindowMatcher, KWinWindowMatcher, WindowsWindowMatcher,
        detect_platform_backend, make_window_matcher, platform_runner_label,
    )
except ImportError:
    from platform_compat import (  # type: ignore[no-redef]  # noqa: F401
        read_process_commandline, looks_like_proton_diablo_window, format_window_debug,
        play_notification_sound, set_dpi_awareness,
        ActiveWindowMatcher, KWinWindowMatcher, WindowsWindowMatcher,
        detect_platform_backend, make_window_matcher, platform_runner_label,
...
def main() -> int:
    ...
    if args.gui:
        gui_script = Path(__file__).with_name("d3keyhelper_gui.py")
    ...
    capture_backend = detect_platform_backend(args.capture_backend)
    matcher = None
    if general.d3only:
        matcher = make_window_matcher(
    ...
    platform_label = platform_runner_label()
